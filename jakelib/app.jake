...
namespace('app', function() {
	...
	task('clobber', function() {
		...
		fs.removeSync(harnessAppPath/*, true*/); // FIXME How do we fail silently with fs-extra?
		fs.mkdirsSync(harnessAppPath, { mode: 0777 });
	...
	task('setup', ['app:clobber'], function() {
		...
		if (!path.existsSync(resourcesPath)) {
			fs.mkdirsSync(resourcesPath, { mode: 0777 });
		}
		require('child_process').exec('alloy new -f "' + harnessAppPath + '"', function(error, stdout, stderr) {
			if (error) {
				...
				fs.copySync(path.join(process.cwd(), 'test', 'apps', appDir), targetAppPath);
				fs.mkdirsSync(path.join(targetAppPath,'lib'), { mode: 0777 });
				fs.copySync(
					path.join('test','lib'),
